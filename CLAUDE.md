...
**API tracking (issue #275, live v104).** Latest spec `api-specs/v104.yaml`. The catch-up v96→v104 is **phased**, and only **phase 1 (additive tracking) has landed** — the cockpit's *behaviour* still tracks v96:
...
- **Phase 2 (todo)** — rate limiting: per-token sliding window, `X-RateLimit-*` headers, **429** + `Retry-After` (`rate_limit_exceeded`); a 429 must not read as a link failure at the preflight.
