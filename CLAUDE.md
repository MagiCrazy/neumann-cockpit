...
**API tracking (issue #275, live v104).** Latest spec `api-specs/v104.yaml`. The catch-up v96→v104 is **phased**; phases 1-3 have landed, the interactive surfaces still track v96:
...
- **Phase 5 (todo)** — `GET /api/visited-sectors` (player-wide union) for the fleet-wide picker and the map; `status=unread` filters on alerts / damage-warnings / messages.
- **Phase 6 (todo)** — `POST …/mannies/tasks`, the atomic batch endpoint, behind the script fan-out and the queue lane rounds.
