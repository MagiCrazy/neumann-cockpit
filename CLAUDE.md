...
**API tracking (issue #275, live v104).** Latest spec `api-specs/v104.yaml`. The catch-up v96→v104 is **phased**; phases 1-2 have landed, the interactive surfaces still track v96:
...
- **Phase 3 (todo)** — server-driven polling: feed `nextUsefulRefreshDelayMs` into the refresh deadline, the production queue (#197) and the script sequencer (#198); adopt `GET …/mannies/{mannyId}` when watching a single Manny.
