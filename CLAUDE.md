...
- **Comms pane** (`Enter`) — messaging inbox/sent (mark read, compose to a probe/planet recipient); the message detail view shows the emission sector when the API reports it (`ProbeMessage.sector`, API v71). Alerts + damage-warnings live in the same pane.
